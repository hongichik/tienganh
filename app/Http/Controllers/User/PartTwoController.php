...
class PartTwoController extends Controller
{
    ...
    public function show(Request $request): View
    {
        ...
        if ($queue->isEmpty()) {
            return view('user.part2', [
                'question' => null,
                'remainingCount' => 0,
                'totalCount' => 0,
                'showHint' => false,
                'hintText' => null,
                'feedbackStatus' => session('part2_feedback_status'),
                'feedbackMessage' => session('part2_feedback_message'),
                'answerValue' => '',
                'personalHintValue' => '',
            ]);
        }
    ...
    public function submitAnswer(Request $request): RedirectResponse
    {
        ...
        if ($queue->isEmpty()) {
            return redirect()->route('user.writing.part2');
        }
        ...
        $redirect = redirect()->route('user.writing.part2')
            ->with('part2_feedback_status', $isCorrect ? 'correct' : 'wrong')
            ->with('part2_feedback_message', $isCorrect ? 'Đúng rồi!' : 'Sai rồi, câu này sẽ được hỏi lại sau.');
    ...
    public function savePersonalHint(Request $request): RedirectResponse
    {
        ...
        return redirect()->route('user.writing.part2', ['show_hint' => 1])
            ->with('part2_feedback_status', 'info')
            ->with('part2_feedback_message', 'Đã lưu gợi ý cá nhân. Từ giờ sẽ ưu tiên gợi ý cá nhân cho câu này.');
    }
    ...
    public function restart(Request $request): RedirectResponse
    {
        ...
        return redirect()->route('user.writing.part2')
            ->with('part2_feedback_status', 'info')
            ->with('part2_feedback_message', 'Đã bắt đầu lại Part 2 với thứ tự ngẫu nhiên mới.');
    }
    ...
    {
        return 'part2_queue_user_' . (int) $request->user()->id;
    }

    private function queueTotalSessionKey(Request $request): string
    {
        return 'part2_total_user_' . (int) $request->user()->id;
    }
